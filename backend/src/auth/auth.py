...
def get_token_auth_header():
    ...
    # Check if list contains 2 elements: Bearer and Token
    if len(header_parts) != 2 or not header_parts:
        ...
        raise AuthError({
            'code': 'invalid header',
            'description': 'Authorization header missing Bearer'}, 401)
...
def check_permissions(permission, payload):
    # Raise error if permissions are not included in payload
    if 'permissions' not in payload:
        abort(400)

    # Raise error if given permission string is not in payload's permission
...
def verify_decode_jwt(token):
    ...
    auth0_json = urlopen(f'https://{AUTH0_DOMAIN}/.well-known/jwks.json')
    
    jwks = json.loads(auth0_json.read())

    # Retrieve the decoded header
    unverified_header = jwt.get_unverified_header(token)

    # Check if auth0 has the key id
    ...
    if rsa_key:
        try:
            # Verify token using the rsa_key
            payload = jwt.decode(
                token,
                rsa_key,
